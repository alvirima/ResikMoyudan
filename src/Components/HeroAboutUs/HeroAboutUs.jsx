import React from "react";
import "./HeroAboutUs.css";

const HeroAboutUs = () => {
  return (
    <div className="au-container">
      <div className="au-card">
        <p>
          <strong>Resik Moyudan</strong> adalah platform digital yang mendukung
          program Sedekah Sampah di Dusun Moyudan, Sumberrahayu, Sleman,
          Yogyakarta. Website ini menjadi pusat informasi, edukasi, dan
          pelaporan kegiatan sedekah sampah. Website ini dikembangkan sebagai
          bagian dari kegiatan KKN Tematik UIN Sunan Kalijaga Yogyakarta yang
          berfokus pada pengelolaan sampah.
          <br />
          <br />
          <strong>“Resik Moyudan”</strong> sendiri merupakan nama yang diambil
          dari dua kata, yaitu:
          <br />• <strong>“Resik”</strong> berasal dari bahasa Jawa yang berarti
          bersih
          <br />• <strong>“Moyudan”</strong> adalah nama dusun tempat
          pelaksanaan program KKN ini
          <br />
          <br />
          Dengan nama ini, diharapkan kegiatan KKN Tematik Resik Moyudan dapat
          membantu masyarakat dalam meningkatkan kesadaran lingkungan, mengelola
          sampah dengan bijak, serta menjadikan sampah bernilai sosial melalui
          gerakan Sedekah Sampah.
        </p>
      </div>
    </div>
  );
};

export default HeroAboutUs;
